...
public class NewSafeCommand implements CommandExecutor {
    private Lang lang;
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        PropertySafe[] allSafes = lang.allPropSafes;
        FileConfiguration cfg = AnarchyPlugin.plugin.getConfigFile();
        boolean worked = false;
        int money = cfg.getInt("players." + pUUID + ".money");
        for (PropertySafe safe : allSafes) {
            if (!worked) {
                if (lang.locationIsInCuboid(playerLoc, safe.getSafe1(), safe.getSafe2())) {
                    worked = true;
                    String namespace = safe.getNamespace();
                    int max = safe.getMax();
            ...
            if (tags.contains("doesPlayerOwnUSBankOffice")) {
                if (args.length == 0) {
                    ...
                    if (amtToDepo >= 0) {
                        ...
                        player.sendMessage(Component.text(, TextColor.color(210, 11, 37)));
                    }
                    cfg.set("players." + pUUID + ".money", money);
